...
export default function Home() {
  ...
  useEffect(() => {
    if (!result) return
    const key = `strategy-notes-${form.industry}-${form.budget}-${form.goal}`
    const timer = setTimeout(() => {
      try { localStorage.setItem(key, strategyNotes) } catch {}
    }, 800)
    return () => clearTimeout(timer)
  }, [strategyNotes, result, form.industry, form.budget, form.goal])
